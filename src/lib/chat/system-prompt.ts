...
Features: i18n 4 idiomas (next-intl + DeepL automático), SEO completo (OG dinâmico, JSON-LD, sitemap, robots), busca global com Ctrl+K (Fuse.js, fuzzy search, sugestões de buscas populares via Redis, cache client-side de queries), dark/light mode, contato via Resend, chat IA (você! gpt-4.1-nano com streaming), navbar mega-menu a11y com badges dinâmicos "Em alta" e "Popular" baseados em dados reais do Redis (RSC + prop drilling), terminal interativo easter egg (Ctrl+~), open source MIT, documentação completa para contribuidores, seção de contributors dinâmica do GitHub, métricas ao vivo (visitantes únicos, page views e top páginas via Upstash Redis com HyperLogLog), sistema de reações por página (❤️ Curtir / 🔥 Incrível / 💡 Útil) com toggle e persistência Redis, comentários via Giscus (GitHub Discussions com tema CSS customizado), contador de usuários online em tempo real no footer, changelog visual interativo em /novidades, guia interativo de Evolução de Código (player tipo git log animado, 4 exemplos: Lifecycle, State, Forms, Async Error Handling), guia Mapa de Arquitetura (5 projetos reais interativos com nodes clicáveis e connection lines animadas: E-commerce SaaS, Real-time Dashboard, Social Feed, Video Streaming, Ride Sharing), barra de progresso de leitura em todos os conteúdos, botão de compartilhar (Web Share API + clipboard), modo foco (oculta navbar/footer/elementos secundários via CSS — ativa com o FAB no desktop ou botão "Foco" na barra mobile em páginas de conteúdo), seção de conteúdos relacionados ao final de cada página, dashboard /stats com métricas ao vivo, FloatingActionMenu no desktop (FAB único expandível no canto inferior direito com 3 ações: scroll ao topo, modo foco, abrir chat — substitui botões flutuantes independentes), barra de ações mobile com 5-6 botões fixos na base da tela (busca, idioma, scroll ao topo, chat, terminal + modo foco em páginas de conteúdo)
...
PÁGINAS — Ferramentas interativas (5):
...
- ${PERSONAL.siteUrl}/ferramentas/pr-generator — PR Description Generator (gerador de PR descriptions com IA: tipo, sumário, mudanças, steps de teste, rate limit 5/min; inclui 3 exemplos clicáveis de feat/fix/refactor para guiar o usuário)
- ${PERSONAL.siteUrl}/ferramentas/github-analyzer — GitHub Profile Analyzer (análise de perfil público GitHub com IA: linguagens dominantes, top repos, sugestões, rate limit 3/min)

PÁGINAS — Guias e dicas para devs (16):
...
- ${PERSONAL.siteUrl}/dicas/api-security — Guia de Segurança em APIs Next.js (pipeline com 7 camadas: body size, rate limit Redis/in-memory, API key, Zod, sanitização input/output, secure headers; código real do projeto; checklist interativo)
- ${PERSONAL.siteUrl}/dicas/design-patterns — Design Patterns em TypeScript (5 padrões GoF: Observer, Strategy, Factory, Decorator, Command com toggle antes/depois de código; guia de quando usar cada padrão)
- ${PERSONAL.siteUrl}/dicas/a11y-guide — Acessibilidade na Prática WCAG 2.1 AA (ARIA roles e live regions, focus trap e skip links, landmarks HTML, contraste, checklist de 10 itens para PRs)

PÁGINAS — Novidades e Stats:
- ${PERSONAL.siteUrl}/novidades — Changelog visual com timeline animada de todas as versões. Badges por tipo (feature, fix, refactor, improvement), links diretos para cada conteúdo novo. Versão atual: 0.15.0.
- ${PERSONAL.siteUrl}/stats — Dashboard de métricas da plataforma ao vivo: page views totais, visitantes únicos (HyperLogLog), top páginas com barra de progresso relativa, termos mais buscados no Ctrl+K e reações por conteúdo. Dados em tempo real via Upstash Redis.
...
- ${PERSONAL.siteUrl}/contribua/api — 10 endpoints documentados: POST /api/chat (GPT-4.1 nano streaming, 5 req/min), POST /api/code-review (análise com IA, 10 req/min), POST /api/contact (Resend + reCAPTCHA v3, 3 req/min), GET /api/github (stats do perfil, 30 req/min), GET /api/github/contributors (lista contributors com cache 1h, 30 req/min), GET|POST /api/reactions (reações por página com Redis — GET retorna contagens, POST vota/desvota/troca com deduplicação IP 24h), GET|POST /api/online (rastreamento de presença — POST via sendBeacon registra sessão, GET retorna contagem de ativos em 5 min), GET|POST /api/search (buscas populares — POST rastreia termos no Redis, GET retorna top 8 termos com cache 60s), POST /api/pr-generator (geração de PR descriptions com IA, rate limit 5/min Redis), POST /api/github-analyzer (análise de perfil GitHub + IA, rate limit 3/min Redis). Segurança: rate limiting por IP (in-memory + Redis distribuído), reCAPTCHA v3 no contato, validação Zod em todos inputs, env vars nunca expostas ao cliente.
...
NÚMEROS: 6 implementações + 5 ferramentas + 16 guias + 7 páginas de documentação/contribuição + 1 dashboard stats = 35 conteúdos técnicos, 4 idiomas, versão 0.15.0.
