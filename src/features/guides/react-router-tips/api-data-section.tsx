import { Cloud, GitPullRequestDraft } from "lucide-react";
...
type ApiDataItem = (typeof reactRouterTipsPage)["apidata"]["items"][number];

export function ApiDataSection() {
  const t = useTranslations("reactRouterTipsPage");

  return (
    <section id="api-data" className="px-6 py-12 md:py-20">
      <div className="mx-auto max-w-5xl">
        <AnimatedSection>
          <SectionHeader
            icon={Cloud}
            title={t("apidata.title")}
            subtitle={t("apidata.subtitle")}
          />
        </AnimatedSection>
        ...
        <div className="flex flex-col gap-6 md:gap-10">
          {(t.raw("apidata.items") as ApiDataItem[]).map((item, i) => (
